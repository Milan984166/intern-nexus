...
class CategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $categories = Category::where('parent_id',0)->orderBy('order_item')->get();
        return view('admin.categories', array('categories' => $categories, 'id' => '0'));
    }

    public function sub_categories($parent_id)
    {
        $category = Category::where('id', base64_decode($parent_id))->firstOrFail();
        $categories = Category::where('parent_id', base64_decode($parent_id))->orderBy('order_item')->get();
        return view('admin.categories', array('categories' => $categories, 'category' => $category, 'id' => '0'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $this->resize_crop_images(1349, 654, $image, $folderPath . "/thumbs/cover_" . $filename);
            $this->resize_crop_images(400, 250, $image, $folderPath . "/thumbs/thumb_" . $filename);
        ...
        return redirect()->to($request->url)->with('status', 'Category Added Successfully!');
    }
    ...
    public function delete($id)
    {
        ...
        if ($category) {
        	// remaining: check if this category has products or not?
            // $categoryProducts = Product::all();
            // foreach ($categoryProducts as $prod) {
            //     if (in_array($id, json_decode($prod['categories']))) {
            ...
            if ($category->delete()) {
	            ...
	            Storage::delete($folderPath ."/thumbs/cover_".$oldImage);
